...
// #Security: 登录暴力破解防护 - 内存级速率限制 (Worker 实例级别)
const 登录频率限制Map = new Map();
const 登录频率限制窗口 = 10 * 60 * 1000; // 10 分钟窗口
const 登录频率限制最大尝试 = 8; // 10分钟内最大尝试次数
const 登录频率限制封禁时间 = 30 * 60 * 1000; // 封禁30分钟

// #Security: 检查登录频率限制
async function 检查登录频率限制(env, ip) {
	const 现在 = Date.now();
	// 1. 内存检查（快速路径）
	const 记录 = 登录频率限制Map.get(ip);
	if (记录) {
		if (记录.封禁直到 && 现在 < 记录.封禁直到) {
			return { 允许: false, 剩余封禁秒: Math.ceil((记录.封禁直到 - 现在) / 1000) };
		}
		// 清理过期记录
		if (现在 - 记录.窗口开始 > 登录频率限制窗口) {
			登录频率限制Map.delete(ip);
		}
	}
	// 2. KV 检查（持久化路径）
	if (env.KV && typeof env.KV.get === 'function') {
		try {
			const kv记录 = await env.KV.get(`brute:${ip}`, { type: 'json' });
			if (kv记录) {
				if (kv记录.封禁直到 && 现在 < kv记录.封禁直到) {
					登录频率限制Map.set(ip, kv记录);
					return { 允许: false, 剩余封禁秒: Math.ceil((kv记录.封禁直到 - 现在) / 1000) };
				}
				if (现在 - kv记录.窗口开始 > 登录频率限制窗口) {
					await env.KV.delete(`brute:${ip}`);
				}
			}
		} catch (e) { /* KV 读取失败不影响登录 */ }
	}
	return { 允许: true };
}

// #Security: 记录登录失败
async function 记录登录失败(env, ctx, ip) {
	const 现在 = Date.now();
	let 记录 = 登录频率限制Map.get(ip);
	if (!记录 || 现在 - 记录.窗口开始 > 登录频率限制窗口) {
		记录 = { 窗口开始: 现在, 尝试次数: 0, 封禁直到: null };
	}
	记录.尝试次数++;
	if (记录.尝试次数 >= 登录频率限制最大尝试) {
		记录.封禁直到 = 现在 + 登录频率限制封禁时间;
	}
	登录频率限制Map.set(ip, 记录);
	// 同步到 KV
	if (env.KV && typeof env.KV.put === 'function' && ctx) {
		ctx.waitUntil(env.KV.put(`brute:${ip}`, JSON.stringify(记录), { expirationTtl: Math.ceil(登录频率限制封禁时间 / 1000) }));
	}
}

// #Security: 清除登录失败记录
async function 清除登录失败记录(env, ctx, ip) {
	登录频率限制Map.delete(ip);
	if (env.KV && typeof env.KV.delete === 'function' && ctx) {
		ctx.waitUntil(env.KV.delete(`brute:${ip}`));
	}
}
...
export default {
	async fetch(request, env, ctx) {
		...
		// #Security: 管理员密码仅从 ADMIN 环境变量获取，不再回退到 KEY/UUID 等非安全变量
		const 管理员密码 = env.ADMIN || '';
		if (!管理员密码) {
			// 兼容旧配置：提示用户迁移
			const 旧密码 = env.admin || env.PASSWORD || env.password || env.pswd;
			if (旧密码) console.warn('[安全警告] 请将环境变量 ADMIN 设置为管理员密码，旧变量名已不再支持');
		}
		...
		if (访问路径 === 'version' && url.searchParams.get('uuid') === userID) {// 版本信息接口
			...
			} else if (访问路径 === 'health') {// #Health: 健康检查端点
				return new Response(JSON.stringify({
					status: 'ok',
					version: Version,
					timestamp: new Date().toISOString(),
					kv: !!(env.KV && typeof env.KV.get === 'function'),
					d1: !!(env.DB && typeof env.DB.prepare === 'function'),
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (request.method === 'POST') {
						// #Security: 登录频率限制
						const 频率检查 = await 检查登录频率限制(env, 访问IP);
						if (!频率检查.允许) return new Response(JSON.stringify({ success: false, error: `尝试过于频繁，${频率检查.剩余封禁秒}秒后重试` }), {
							status: 429, headers: { 'Content-Type': 'application/json;charset=utf-8', 'Retry-After': String(频率检查.剩余封禁秒) }
						});
						...
						if (输入密码 === (typeof 管理员密码 === 'string' ? 管理员密码.replace(/[\r\n]/g, '') : 管理员密码)) {
							// 密码正确，清除失败记录
							ctx.waitUntil(清除登录失败记录(env, ctx, 访问IP));
							const 响应 = new Response(JSON.stringify({ success: true }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
							响应.headers.set('Set-Cookie', `auth=${await MD5MD5(UA + 加密秘钥 + 管理员密码)}; Path=/; Max-Age=2592000; HttpOnly; Secure; SameSite=Lax`);
							return 响应;
						}
						// 密码错误，记录失败
						ctx.waitUntil(记录登录失败(env, ctx, 访问IP));
					}
					...
					if (访问路径 === 'admin/init') {// 重置配置为默认值
						...
						if (!本地优选IP || 本地优选IP === 'null') 本地优选IP = (await 生成随机IP(request, config_JSON.优选订阅生成.本地IP库.随机数量, config_JSON.优选订阅生成.本地IP库.指定端口))[1];
					...
					return new Response(admin响应.body, { status: admin响应.status, statusText: admin响应.statusText, headers: admin头 });
