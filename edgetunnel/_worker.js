// edgetunnel v2.1 - Built from src/ modules
// Source: https://github.com/lalal1711/edgetunnel.gaidong
// Build time: 2026-06-11T07:00:58.375Z
...
const 登录频率限制窗口 = 10 * 60 * 1000;
const 登录频率限制最大尝试 = 8;
const 登录频率限制封禁时间 = 30 * 60 * 1000;

// WS / streaming constants
const WS早期数据最大字节 = 8 * 1024;
const WS早期数据最大头长度 = Math.ceil(WS早期数据最大字节 * 4 / 3) + 4;
const 上行合包目标字节 = 16 * 1024;
const 上行队列最大字节 = 16 * 1024 * 1024;
const 上行队列最大条目 = 4096;
const 下行Grain包字节 = 32 * 1024;
const 下行Grain尾部阈值 = 512;
const 下行Grain静默毫秒 = 0;
...
// ============================================================
// src/crypto.js - Cryptographic functions extracted from _worker.js
// TLS protocol constants, crypto primitives, SS AEAD, ChaCha20/Poly1305
// ============================================================
...
const xorSequenceIntoIv = (initializationVector, sequenceNumber) => {
...
const deriveTrafficKeys = (hash, secret, keyLen, ivLen) => Promise.all([hkdfExpandLabel(hash, secret, "key", EMPTY_BYTES, keyLen), hkdfExpandLabel(hash, secret, "iv", EMPTY_BYTES, ivLen)]);
const TLS_MAX_PLAINTEXT_FRAGMENT = 16 * 1024;

// ---- Data Conversion Helpers (used by many functions) ----
...
// ---- SS AEAD Configuration & Constants ----
const SS支持加密配置 = {
...
// ---- TLS Parser Classes (used by TLS functions) ----
...
// ---- TLS ServerHello / KeyExchange / Certificate Parsers ----
...
const 木马文本解码器_常量 = 木马文本解码器;
const SS文本解码器_常量 = SS文本解码器;
const 幻数常量_val = 幻数常量;
const VLESS文本解码器_常量 = VLESS文本解码器;
const IPv6方括号正则_常量 = IPv6方括号正则;
const 反代协议默认端口_常量 = 反代协议默认端口;
const 查杀特征码_常量 = 查杀特征码;
...
// ===== Module: src/queue.js =====
// ============================================================
// src/queue.js - Upload queue, downstream grain sender, stream connector
// ============================================================
...
function 创建上行写入队列({ 获取写入器, 释放写入器, 重试连接, 关闭连接, 名称 = '上行队列' }) {
	...
	const drain = async () => {
		...
		try {
			...
			closed = true;
			clear(err);
			if (Config.调试日志打印) console.log(`[${名称}] 写入失败: ${err?.message || err}`);
	...
	const enqueue = (data, allowRetry = true, waitForFlush = false) => {
		...
		if (nextBytes > 上行队列最大字节 || nextItems > 上行队列最大条目) {
			...
			clear(err);
			if (Config.调试日志打印) console.log(`[${名称}] 队列超限，关闭连接`);
			try { 关闭连接?.(err) } catch (_) { }
			throw err;
		}
 ...
 */
function createSSContextFactory({ yourUUID, url, serverSock, closeSocketQuietly, WebSocket发送并等待 }) {
	let ss上下文 = null;
	let ss初始化任务 = null;
 ...
 */
async function 处理SS数据(chunk, ssContext, deps) {
...
// ===== Module: src/html.js =====
// ============================================================
// src/html.js - HTML decoy/fake pages
// ============================================================
...
// ============================================================
// src/subscription.js - Subscription content patching functions
// ============================================================
// ---- Text Decoders ----
const 木马文本解码器 = new TextDecoder();
const VLESS文本解码器 = new TextDecoder();
